...
const specs: Record<SolidKind, SolidSpec> = {
  ...
  box: {
    geometry: () => new THREE.BoxGeometry(3.2, 2, 2.4),
    dimensions: { width: 3.2, height: 2, depth: 2.4 },
    height: 2,
    vertices: () => rectangleVertices(3.2, 2, 2.4),
  },
  cylinder: {
    geometry: () => new THREE.CylinderGeometry(1.2, 1.2, 2.8, 72, 1),
    dimensions: { width: 2.4, height: 2.8, depth: 2.4, radius: 1.2 },
    height: 2.8,
    vertices: () => [
      ...radialVertices(1.2, 0, 'bottom'),
      ...radialVertices(1.2, 2.8, 'top'),
      { id: 'bottom-center', label: '底面中心', position: new THREE.Vector3(0, 0, 0) },
      { id: 'top-center', label: '上面中心', position: new THREE.Vector3(0, 2.8, 0) },
    ],
  },
  cone: {
    geometry: () => new THREE.ConeGeometry(1.4, 2.8, 72, 1),
    dimensions: { width: 2.8, height: 2.8, depth: 2.8, radius: 1.4 },
    height: 2.8,
    vertices: () => [
      ...radialVertices(1.4, 0, 'bottom'),
      { id: 'apex', label: '頂点', position: new THREE.Vector3(0, 2.8, 0) },
      { id: 'bottom-center', label: '底面中心', position: new THREE.Vector3(0, 0, 0) },
    ],
  },
  sphere: {
    geometry: () => new THREE.SphereGeometry(1.2, 72, 36),
    dimensions: { width: 2.4, height: 2.4, depth: 2.4, radius: 1.2 },
    height: 2.4,
    vertices: () => [
      { id: 'bottom', label: '下端', position: new THREE.Vector3(0, 0, 0) },
      { id: 'top', label: '上端', position: new THREE.Vector3(0, 2.4, 0) },
      ...radialVertices(1.2, 1.2, 'middle'),
    ],
  },
};

export function getSolidDimensions(kind: SolidKind): SolidDimensions {
  return specs[kind].dimensions;
}
